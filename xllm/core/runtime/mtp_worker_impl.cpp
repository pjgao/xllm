...
bool MTPWorkerImpl::init_model(const std::string& model_weights_path,
  ...
  if (impl_ != nullptr && impl_->get_status() == WorkerImpl::Status::LOADED) {
    ...
    target_spec_verify_capabilities_ = impl_->speculative_verify_capabilities();
  }
...
SpeculativeVerifyCapabilities MTPWorkerImpl::speculative_verify_capabilities()
    const {
...
std::optional<ForwardOutput> MTPWorkerImpl::step_decode(
  ...
  if (use_prelaunched_first_draft) {
    ...
    // token handoff; it is only needed for draft steps 1..N-1 and target.
...
void MTPWorkerImpl::fill_validate_input_from_draft_outputs(
  ...
  if (use_fused_verify_token_update) {
    ...
    for (const ForwardOutput& draft_output : draft_outputs) {
      ...
      CHECK(next_tokens.defined() && next_tokens.numel() == 1 &&
...
std::optional<ForwardOutput> MTPWorkerImpl::run_validate(
  ...
  StreamEventPtr handoff_ready_event;
  {
    ...
    accepted_tokens_host.copy_(val_output.next_tokens,
                               /*non_blocking=*/true);
    // The event covers consensus, base-state derivation, and the D2H handoff.
    handoff_ready_event = compute_stream_->record_event();
  }
  if (handoff_ready_event == nullptr) {
    const int32_t ret = compute_stream_->synchronize();
    CHECK_EQ(ret, 0) << "failed to synchronize MTP handoff, ret=" << ret;
  }
  ...
  // Keep their prepared inputs alive until the target handoff event completes.
  for (const ForwardOutput& draft_output : draft_outputs) {
...
bool MTPWorkerImpl::supports_combined_first_draft_execution() const {
  ...
  // handoff is rank-local rather than DP-aware.
...
void MTPWorkerImpl::prepare_validate_inputs(const ForwardInput& input,
  ...
  if (use_explicit_spec_verify_replay_update) {
    ...
    const auto host_options = torch::TensorOptions()
                                  ...
                                  .device(torch::kCPU)
                                  .pinned_memory(true);
    if (!spec_verify_tokens_host_.defined() ||
        spec_verify_tokens_host_.numel() != total_num_val_tokens) {
      spec_verify_tokens_host_ =
          torch::empty({total_num_val_tokens}, host_options);
      spec_verify_tokens_device_ =
          torch::empty({total_num_val_tokens}, token_options);
      spec_verify_positions_host_ =
          torch::empty({total_num_val_tokens}, host_options);
      spec_verify_positions_device_ =
          torch::empty({total_num_val_tokens}, position_options);
    }
    std::memcpy(spec_verify_tokens_host_.data_ptr<int32_t>(),
                buf.out_token_ids.data(),
                buf.out_token_ids.size() * sizeof(int32_t));
    std::memcpy(spec_verify_positions_host_.data_ptr<int32_t>(),
                buf.out_positions.data(),
                buf.out_positions.size() * sizeof(int32_t));
    spec_verify_tokens_device_.copy_(spec_verify_tokens_host_,
                                     /*non_blocking=*/true);
    spec_verify_positions_device_.copy_(spec_verify_positions_host_,
                                        /*non_blocking=*/true);
    validate_input.token_ids_host = spec_verify_tokens_host_;
    validate_input.positions_host = spec_verify_positions_host_;
    validate_input.token_ids = spec_verify_tokens_device_;
    validate_input.positions = spec_verify_positions_device_;
    // Expose stable base-token storage to the fused graph input update.
    // Without this binding the fused gate falls back to five casts + stack on
    // every final-draft boundary even though token_ids already has a stable
    // address suitable for ACL graph replay.
    input_params.graph.input_tokens_override = spec_verify_tokens_device_;
    validate_input.device_tensors_ready = true;
#endif
  } else {
  ...
  if (use_explicit_spec_verify_replay_update) {
    ...
    CHECK_EQ(attention.host.block_tables.size(0), 1);
    ...
    if (spec_verify_attention_host_buffer_.defined()) {
    ...
    // The stable buffers are owned by MTPWorkerImpl; this temporary owner must
    // remain unique so rebuild_device_buffer reuses rather than detaches them.
    ...
    CHECK_EQ(expanded_block_rows, num_val_tokens);
    ...
    attention.rebuild_device_buffer(device_, extra_int_inputs);
