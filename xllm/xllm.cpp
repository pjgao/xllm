...
void validate_config(const std::string& model_type) {
  ...
  if (speculative_config.num_speculative_tokens() > 0 &&
      ...
      !SpeculativeConfig::supports_npu_graph_double_buffer(
          model_type,
          speculative_config.speculative_algorithm(),
          execution_config.enable_fia_decode(),
          execution_config.enable_graph_mode_decode_no_padding())) {
    LOG(WARNING) << "enable_graph_double_buffer is only supported for NPU "
                    "Qwen3.5 MTP with FIA and no-padding graph decode. "
